...
END
  ...
  ((test/≠° _)
    (⊦= '((λ (α) (deny (equal? α 1)) α)) (°->list #f (fresh° (s) (≠° s 1))))
    (⊦= '() (°->list #f (fresh° (s) (≠° s 1) (=° s 1))))
    (⊦= '((λ (α) (deny (equal? α (a b))) α)) (°->list #f (fresh° (s) (≠° s '(a b)))))
    (⊦= '((λ (α β) (deny (equal? α 1)) (deny (equal? β 2)) (cons α (cons β (quote ()))))) 
        (°->list #f (fresh° (q p r) (≠° (list p r) '(1 2)) (=° q (list p r)))))
    (⊦= '((λ (α) (deny (equal? α 2)) (cons 1 (cons α (quote ())))))
