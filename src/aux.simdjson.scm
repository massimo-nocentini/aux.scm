  ...
  (define (simdjson->scheme w)
    (let1 (t (simdjson-get-type w))
      (cond
        ...
        ((simdjson-array? t) (simdjson-get-array w make-vector (λ (vec i w*) (vector-set! vec i (simdjson->scheme w*)) vec)))
        ((simdjson-object? t) (vector->list 
                                (simdjson-get-object w make-vector (λ (vec i k w*)
                                  (vector-set! vec i (list k (simdjson->scheme w*))) vec))))
